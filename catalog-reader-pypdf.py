    ...
    if args.extractpage:
        def visitor_body(text, cm, tm, fontDict, fontSize):
            ...
            for ix, container in enumerate(schema):
                if container[0] < y and y <=container[1]:
                    if parsed[ix].get(y, None) is None:
                        parsed[ix][y] = []
                    if text.startswith('Sat 7 Nov'):
                        logging.info("where is this...")
                    parsed[ix][y].append(text)
        ...
        # now process into films...
        for film_parts in parsed:
            film = {'title' : None, 'info' : None, 'blurb': None}
            keys = sorted(film_parts.keys())
            keys.reverse()
            for key in keys:
                text_parts = film_parts[key]
                if film['title'] is None:
                    film['title'] = text_parts
                elif film['info'] is None:
                    film['info'] = text_parts
                else:
                    pass
