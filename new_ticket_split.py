...
def find_owner(schedules, film_name, film_place, film_date, film_time):
    ...
    logging.debug(
        f"Searching for: {film_name} @ {film_date.strftime('%Y-%m-%d')} {film_time.strftime('%H:%M')}"
    )
    for owner, schedule in schedules.items():
        for location in schedule.get("locations", []):
            for event in location.get("events", []):
                ...
                logging.debug(
                    f"  Checking against: {event_name} @ {event_start_str} for {owner}"
                )
                if strip_diacritics(film_name.strip().lower()) in strip_diacritics(
                    event_name
                ):
                    try:
...
def process_pdfs(in_dir, out_dir, schedules):
    ...
    for fname in os.listdir(in_dir):
        ...
        for i, page in enumerate(reader.pages):
            ...
            owner, new_schedules = find_owner(
                schedules, film_name, film_place, film_date, film_time
            )
            ...
            safe_title = "".join(
                c for c in film_name if c.isalnum() or c in " -_"
            ).rstrip()
